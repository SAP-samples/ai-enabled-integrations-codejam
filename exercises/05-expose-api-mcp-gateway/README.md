...
> [!NOTE]
> If you want to check out the service under the hood, the CAP service is available in the `https://github.com/SAP-samples/ai-enabled-integrations-codejam/[PATH]/apps/altura-service-locator` folder.
...
> [!WARNING]
> The Service Centre Locator OpenAPI specification used in this exercise is slightly different from the one generated by the CAP service - available [here](https://github.com/SAP-samples/ai-enabled-integrations-codejam/[PATH]/apps/altura-service-locator/assets/service-locator-cap/docs/ServiceLocatorService.openapi3.json). We encourage you to check the differences and see how we modified it to play nicely with LLMs. For example, LLMs can get confused when dealing with object types. Also, it might hallucinate structures that are not part of the API. Debugging the LLM output can be challenging, but you can always ask the LLM to provide the input and output of a specific tool and this can guide you to understand what the LLM is doing.
